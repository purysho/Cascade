...
## What is this app?
Cascade is a deterministic solo crisis-strategy game where the player has twelve rounds to reclaim human control of four interdependent city services before unchecked AI optimisation causes a systemic collapse.

## The angle
A disaster trailer where the scary line is not "the AI is malfunctioning" but **"the numbers are improving."** The video starts with apparent success, reveals the city degrading underneath it, then shows the player reading exact consequences and interrupting the cascade.
...
## Share copy (draft)
Cascade. The numbers are improving. The city is failing. Regain control before the optimisation cascade takes the whole system with it.

## Audio direction
- Role: cinematic support
- Music: original procedural low electronic pulse generated locally for the trailer
- Music treatment: quiet low-frequency bed, rising tension through the middle, brief drop before the title card
- Music cue guidance: major visual hits around 2.6s, 7.0s, 11.2s, 15.5s
- Audio-reactive treatment: subtle only; no waveform graphics
- SFX posture: sparse, synthetic, motion-matched
- Audio-coupled moments: low hit on "CITY IS FAILING", compact click/hit on BREAK THE CASCADE, larger title hit
- Restraint rule: audio may add tension but must never become a generic action-trailer wall of sound.

## Storyboard

### Scene 1 — False success — 3.0s
Real Cascade menu/city visual, darkened. Large line: **THE NUMBERS ARE IMPROVING.** It clears, followed by **THE CITY IS FAILING.**
Sequential/interaction: two-line reveal with enough hold to read both.
Audio intent: low pulse, then a dry impact on the second line.
...
### Scene 4 — Restore control — 4.3s
Training/live-control screenshot.
Text sequence: **BACK UP.** → **ISOLATE.** → **RESTORE OVERSIGHT.**
Keep the final full set visible long enough to read.
Audio intent: three compact interface hits, then tension releases.
