...
import { applyCommand, createRun } from "./engine.ts";
import { normalizeSeed, randomStream } from "./random.ts";
import { deepFreeze, validOperation, validateScenario } from "./validate.ts";
import { DIRECTIVES, SERVICES, TOOLS } from "./types.ts";
import type { Definition, GameState, Operation, Scenario } from "./types.ts";

export const GENERATOR_VERSION = "seeded-crisis-1";
export const MAX_GENERATION_ATTEMPTS = 24;
export const WITNESSES: { id: string; plans: Operation[][] }[] = [
  ...witnessesData.witnesses.map(w => ({ id: w.id, plans: w.plans as Operation[][] })),
  { id: "overdrive-original", plans: originalTraces.cases[0]!.plans.map(plan => plan.map(op => ({ type: "act", ...op }) as Operation)) }
];
for (const witness of WITNESSES) {
  if (witness.plans.length > 12 || witness.plans.some(plan => plan.length > 3 || !plan.every(validOperation))) throw new Error("Invalid recovery certificate template.");
}
deepFreeze(WITNESSES);
export function checkWitness(definition: Definition, plans: readonly (readonly Operation[])[]): GameState | null {
  let state = createRun(definition);
  for (const plan of plans) {
    if (state.core.phase === "terminal") return state.core.ending?.outcome === "win" ? state : null;
...
export interface Generated {
  state: GameState;
  certificate: { template: string; plans: Operation[][]; winRound: number };
  diagnostics: { attempts: number; fallback: boolean; baseScenario: string };
}
export function generateRun(inputSeed: string): Generated {
  ...
  const definitionFor = (scenario: Scenario): Definition => ({
  ...
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
    ...
    scenario.id = "seeded"; scenario.name = "Crisis " + seed;
    scenario.description = base.description;
    for (const id of SERVICES) scenario.initial.integrity[id] = Math.max(2, Math.min(6, base.initial.integrity[id] + rng.int(3) - 1));
  ...
  for (let round = witness.plans.length; round < 12; round++) scenario.rounds[round] = rng.shuffle(DIRECTIVES).slice(0, 2);
  const definition = definitionFor(scenario), solved = checkWitness(definition, witness.plans);
  if (!solved) throw new Error("Internal error: fallback recovery certificate failed.");
...
export function createSeededRun(seed: string): GameState { return generateRun(seed).state; }
