import { ACTIONS, DIRECTIVES, SERVICES, TOOLS } from "./types.ts";
import type { Command, Operation, Rules, Scenario } from "./types.ts";
...
export function member<T extends string>(list: readonly T[], value: unknown): value is T {
...
export function validateScenario(value: unknown): asserts value is Scenario {
  ...
  for (const id of SERVICES) requireValid(integer(initial.integrity[id], 0, 6), "integrity " + id);
...
export function validateRules(value: unknown): asserts value is Rules {
  ...
  for (const n of Object.values(value.constants)) requireValid(integer(n, 0, 20), "bounded integer constants");
  ...
  for (const d of value.directives) {
    ...
    for (const [id, n] of Object.entries(d.integrityDelta)) requireValid(member(SERVICES, id) && integer(n, -6, 6), "delta");
    ...
    for (const delay of d.delayed) requireValid(record(delay) && delay.afterRounds === 1
  ...
  for (const k of ["fieldPatchGain", "supplyDropGain", "civicReliefGain", "surgeSupportGain"]) requireValid(integer(roguelike[k], 1, 6), "tool magnitude");
...
export function validOperation(value: unknown): value is Operation {
...
export function validCommand(value: unknown): value is Command {
  if (!record(value) || !integer(value.expectedRevision, 0, 1_000_000)) return false;
  const { expectedRevision: _, ...command } = value;
  if (command.type === "undo" || command.type === "commit_round") return keys(command, ["type"]);
  if (command.type === "choose_tool") return keys(command, ["type", "tool"]) && (command.tool === null || member(TOOLS, command.tool));
...
export function fingerprint(value: unknown): string {
  ...
  for (const byte of new TextEncoder().encode(input)) h = BigInt.asUintN(64, (h ^ BigInt(byte)) * 0x100000001b3n);
