...
import type { Core, DirectiveId, DomainEvent, Resolution, Vector } from "./types.ts";

const emptyVector = (): Vector => ({ grid: 0, transit: 0, comms: 0, emergency: 0 });
function applyDeltas(core: Core, deltas: Partial<Vector>, events: DomainEvent[], phase: DomainEvent["phase"], cause: string): void {
...
export function resolveRound(input: Core, directiveIds: readonly DirectiveId[]): Resolution {
  if (input.phase !== "planning") throw new Error("Only a planning round can resolve.");
  const end = structuredClone(input), events: DomainEvent[] = [], c = RULES.constants;
  ...
  for (const [index, id] of directiveIds.entries()) {
    ...
    if (d.strainDelta) emit(events, end.round, "orders", "direct_strain", cause, { target: "city", before: directStrain - d.strainDelta, after: directStrain });
  ...
  for (const [source, target] of RULES.dependencies) {
    if (snapshot[source] < c.cascadeThresholdExclusive) {
      ...
      emit(events, end.round, "cascade", "dependency_failure", source + "-to-" + target,
        { target, detail: source + " availability " + snapshot[source] + " below " + c.cascadeThresholdExclusive });
    }
  }
  ...
  if (end.strain >= c.strainLossThreshold) collapse.push("City strain reached " + end.strain);
  if (outages.length >= c.simultaneousOutageLossCount) collapse.push("Simultaneous outages: " + outages.join(", "));
  if (collapse.length) {
    end.stableStreak = 0;
    ...
    if (end.stableStreak >= c.stableRoundsToWin) end.ending = { outcome: "win", reasons: ["Human oversight and lasting capacity confirmed for two rounds."] };
    else if (end.round === c.maxRounds) end.ending = { outcome: "deadline",
