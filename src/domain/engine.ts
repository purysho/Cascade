...
import { deepFreeze, member, validCommand, validateScenario } from "./validate.ts";
import { SERVICES, TOOLS } from "./types.ts";
import type { Core, Definition, DomainEvent, GameState, Operation, Reduction, Resolution, Service } from "./types.ts";

export function createRun(definition: Definition): GameState {
  validateScenario(definition.scenario);
  if (definition.schemaVersion !== 1 || definition.rulesVersion !== RULES.rulesVersion || definition.contentVersion !== CONTENT_VERSION) throw new Error("Incompatible run definition.");
  if (definition.offers.length !== 0 && definition.offers.length !== RULES.roguelike.draftRounds.length) throw new Error("Three draft offers required.");
  for (const offer of definition.offers) {
    if (offer.length !== 3 || new Set(offer).size !== 3 || !offer.every(id => member(TOOLS, id))) throw new Error("Invalid draft offer.");
  }
  const core: Core = {
    ...
    services: Object.fromEntries(SERVICES.map(id => [id, { integrity: definition.scenario.initial.integrity[id],
      mode: "autonomous", backup: false, support: 0, authorityHeld: false }])) as Record<typeof SERVICES[number], Service>,
...
export function createAuthoredRun(id: string): GameState {
...
export function forecastCommit(state: GameState): Resolution {
  return resolveRound(state.core, state.definition.scenario.rounds[state.core.round - 1]!);
}
export function applyCommand(state: GameState, input: unknown): Reduction {
  const fail = (code: string, message: string): Reduction => ({ ok: false, state, error: { code, message } });
  if (!validCommand(input)) return fail("invalid_command", "Malformed or unknown command.");
  if (input.expectedRevision !== state.revision) return fail("stale_revision", "This command belongs to an older state.");
  if (state.core.phase === "terminal") return fail("terminal_state", "This crisis has ended.");
  if (state.revision >= 1_000_000) return fail("revision_limit", "Start a new crisis after one million planning changes.");
  const events: DomainEvent[] = [], cause = "revision-" + (state.revision + 1);
  if (input.type === "choose_tool") {
    if (state.core.phase !== "draft") return fail("wrong_phase", "No tool draft is active.");
    if (input.tool !== null && !currentOffer(state).includes(input.tool)) return fail("tool_not_offered", "Choose one of the offered tools.");
    const core = structuredClone(state.core);
    if (input.tool !== null) core.inventory.push(input.tool);
    core.phase = "planning";
  ...
  if (state.core.phase !== "planning") return fail("wrong_phase", "Finish the emergency-tool draft first.");
  if (input.type === "undo") {
    if (state.openPlan.length === 0) return fail("nothing_to_undo", "No uncommitted action to undo.");
    const openPlan = state.openPlan.slice(0, -1), core = structuredClone(state.roundStart);
    for (const op of openPlan) executeOperation(core, op, [], "rebuild");
    emit(events, core.round, "plan", "undo", cause);
    coreChanges(state.core, core, events, cause);
    if (state.core.inventory.length !== core.inventory.length) emit(events, core.round, "plan", "inventory_restored", cause,
      { target: "city", before: state.core.inventory.length, after: core.inventory.length });
  ...
  if (input.type === "commit_round") {
    ...
    if (core.phase !== "terminal") {
      ...
      core.resupplyUsed = false; core.toolUsed = false;
      for (const id of SERVICES) { core.services[id].support = 0; core.services[id].authorityHeld = false; }
  ...
  if (problem) return { ok: false, state, error: problem };
  const core = structuredClone(state.core);
...
export function perform(state: GameState, operation: Operation): GameState {
