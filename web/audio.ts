...
export type UiCue = "action" | "undo" | "draft" | "confirm";
...
export interface AudioController {
  ...
  playResolution(events: readonly DomainEvent[]): void;
}

export function createAudioController(initialEnabled = true): AudioController {
  ...
  function noise(duration: number, offset = 0, gain = 0.13): void {
    const audio = ensureContext();
    if (!audio || !master) return;
  ...
  function playUi(cue: UiCue): void {
    if (!enabled) return;
    if (cue === "action") {
  ...
  function playResolution(events: readonly DomainEvent[]): void {
