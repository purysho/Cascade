...
export interface TutorialSpec {
  id: "read_danger" | "break_cascade" | "contain_safely" | "restore_control";
  title: string;
  task: string;
  focus: ServiceId;
...
const emptyRounds = (): Scenario["rounds"] => Array.from({ length: 12 }, () => []);
...
export function createTutorialState(id: TutorialSpec["id"]): GameState {
  const spec = TUTORIALS.find(item => item.id === id);
  if (!spec) throw new Error("Unknown tutorial: " + id);
